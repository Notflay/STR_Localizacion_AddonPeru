...
lv_union CHAR(1);
BEGIN
lv_union := CASE WHEN :Tpo = 'DP' THEN NULL 
				 WHEN :Tpo = 'D'  THEN 0
				 WHEN :Tpo = 'P'  THEN 1
...
ProvD  := NULLIF(:ProvD,  '');
ProvH  := NULLIF(:ProvH,  '');
FcCntD := NULLIF(:FcCntD, '');
FcCntH := NULLIF(:FcCntH, '');
FcVncD := NULLIF(:FcVncD, '');
FcVncH := NULLIF(:FcVncH, '');
Moneda := NULLIF(:Moneda, '');
...
SELECT
	...
	WHERE IFNULL(T1."U_BPP_AstDetrac",'') = '' AND LEFT(T2."WTCode",1) = 'D' AND T1."CANCELED" = 'N'
...
SELECT
	...
	FROM 
		(SELECT * FROM OINV UNION ALL
		 ...
		 SELECT * FROM ORIN) T1
	WHERE IFNULL(TO_VARCHAR(T1."U_BPP_TtPc"),'')<>'' AND T1."U_BPP_TtPc" > 0.0 
	  AND IFNULL(T1."U_BPP_AsPc",'')='' AND T1."CANCELED" = 'N'
...
SELECT DISTINCT 
	...
	,T1."TransId"			AS "CodTransaccion"
	,T1."DocEntry"			AS "DocEntry"
	,'Acticipo de Clientes' AS "TipoDocumento"
	,T1."U_BPP_MDTD"||'-'||T1."U_BPP_MDSD"||'-'||T1."U_BPP_MDCD" AS "NroDocumento"
	,T1."TaxDate"			AS "FechaDocumento"
	...
	FROM ODPI T1 WHERE T1."DocStatus" = 'O' 
		AND T1."DocEntry" NOT IN (SELECT IFNULL(TS0."U_STR_BsEnt",'0') FROM JDT1 TS0 WHERE  TS0."U_STR_BsTyp" = T1."ObjType") 
...
SELECT DISTINCT
	...
	FROM OINV T1 WHERE T1."DocStatus" = 'O' 
		AND T1."DocEntry" NOT IN (SELECT DISTINCT IFNULL(TS0."U_STR_BsEnt",'0') FROM JDT1 TS0 LEFT JOIN OJDT TS1 ON TS0."TransId" = TS1."StornoToTr"
